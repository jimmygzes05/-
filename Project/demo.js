...
function readOsu() {
    ...
    reader.onload = function (f) {
        ...
        for (let i = 0; i < TPnums; i++)
            if (content["TimingPoints"][i][1] < 0) content["TimingPoints"][i][8] = BPMnums - 1;
            else {
...
function calcPOS(tt) {
    ...
    for (let i = p1; i < TPnums; i++) {
        ...
        if (content["TimingPoints"][i][1] > 0) percent = 1;
        else percent = -content["TimingPoints"][i][1] / 100;
...
function bt_start_onclick() {
    ...
    if (document.getElementById("baseBPM").value !== "") baseMpB = 60000 / Number(document.getElementById("baseBPM").value);
    else {
        baseMpB = Number(content["TimingPoints"][0][1]);
        for (let i = 1; i < TPnums; i++)
            if (Number(content["TimingPoints"][i][1]) > 0) baseMpB = (baseMpB < Number(content["TimingPoints"][i][1]) ? baseMpB : Number(content["TimingPoints"][i][1]));
    }
    initKeyListener();
    startTime();
}
...
function processTouchstart(e) {
    ...
    let Key = 0,
        Finded = false;
    if (X <= 150 * Scale / 100) Key = 0;
    else if (X <= 300 * Scale / 100) Key = 1;
    else if (X <= 450 * Scale / 100) Key = 2;
    else Key = 3;
    if (!keyAsync[Key]) {
        keyAsync[Key] = true;
        keyLaserTime[Key] = -1;
...
function draw_Lasers() {
    ...
    for (let Col = 0; Col < 4; Col++) {
        ...
        let Width = 0;
        if (keyLaserTime[Col] === -1) Width = 150;
        else
